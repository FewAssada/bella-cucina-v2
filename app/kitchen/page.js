...
export default function KitchenPage() {
  ...
  // --- Table Actions (หัวใจสำคัญ: สร้างรหัสลับ) ---
  const toggleTableActive = async (id, currentStatus) => {
    const newStatus = !currentStatus;
    
    // ถ้าเปิดโต๊ะใหม่ -> สร้างรหัสลับ (ใช้เวลาปัจจุบันเป็นตัวเลขสุ่ม)
    // ถ้าปิดโต๊ะ -> ลบรหัสทิ้ง
    const newToken = newStatus ? Date.now().toString() : null;
