...
export default function AdminPage() {
  ...
  const fetchData = async () => {
    ...
    // 🔥 ดึงค่า Settings
    const { data: s } = await supabase.from('restaurant_settings').select('*').single();
    if (s) setSettings(s);
  ...
  const handleSaveSettings = async () => {
      ...
      // อัปเดตแถวแรกเสมอ (id=1 หรือแถวแรกที่เจอ)
      const { data: existing } = await supabase.from('restaurant_settings').select('id').single();
      if (existing) {
          await supabase.from('restaurant_settings').update(settings).eq('id', existing.id);
      } else {
          await supabase.from('restaurant_settings').insert([settings]);
      }
    ...
    <div className="min-h-screen bg-[#F3F4F6] font-sans flex text-gray-800">
      ...
      <aside className="w-20 lg:w-64 bg-white border-r border-gray-200 flex-shrink-0 flex flex-col fixed h-full z-20 transition-all">
          <div className="p-6 flex items-center gap-3 justify-center lg:justify-start">
              ...
              <div className="hidden lg:block"><h1 className="font-bold text-lg leading-tight">{settings.shop_name || 'My POS'}</h1><p className="text-xs text-gray-400">ระบบจัดการร้านอาหาร</p></div>
          </div>
      ...
      <main className="flex-1 ml-20 lg:ml-64 p-4 lg:p-8 overflow-y-auto min-h-screen">
          ...
          <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
              <div>
                  <h2 className="text-2xl font-bold text-gray-800">
                      {activeTab === 'dashboard' ? 'แดชบอร์ดภาพรวม' : activeTab === 'pos' ? 'ขายหน้าร้าน (POS)' : activeTab === 'kitchen' ? 'รายการออเดอร์ (ครัว)' : activeTab === 'menu' ? 'จัดการเมนูอาหาร' : 'ตั้งค่าระบบ'}
                  </h2>
                  {activeTab === 'dashboard' && (
                      <div className="flex items-center gap-2 mt-1"><span className="text-sm text-gray-500">📅 ประจำวันที่:</span><input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-1 outline-none focus:border-orange-500 cursor-pointer shadow-sm hover:border-orange-300 transition-colors"/></div>
                  )}
              </div>
              <div className="flex gap-2">
                  <button onClick={() => speak("เทส")} className="bg-white p-2 rounded-full shadow-sm text-gray-500 hover:text-orange-600">🔊</button>
                  <button onClick={stopSpeaking} className="bg-white p-2 rounded-full shadow-sm text-gray-500 hover:text-red-600">🔇</button>
              </div>
          </header>
              ...
              <div className="animate-fade-in">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                          <div className="flex justify-between items-start mb-4"><div className="p-3 bg-green-100 text-green-600 rounded-xl">💰</div></div>
                          <h3 className="text-gray-500 text-sm">ยอดขาย</h3><p className="text-3xl font-bold text-gray-800">฿{totalSalesToday.toLocaleString()}</p>
                      </div>
                      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                          <div className="flex justify-between items-start mb-4"><div className="p-3 bg-blue-100 text-blue-600 rounded-xl">📃</div></div>
                          <h3 className="text-gray-500 text-sm">ออเดอร์สำเร็จ</h3><p className="text-3xl font-bold text-gray-800">{totalOrdersToday}</p>
                      </div>
                      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                          <div className="flex justify-between items-start mb-4"><div className="p-3 bg-orange-100 text-orange-600 rounded-xl">⏳</div></div>
                          <h3 className="text-gray-500 text-sm">กำลังทำ</h3><p className="text-3xl font-bold text-gray-800">{pendingOrders}</p>
                      </div>
                  </div>
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                      <div className="p-6 border-b border-gray-100"><h3 className="font-bold">รายการออเดอร์ล่าสุด</h3></div>
                      <table className="w-full text-left text-sm">
                          <thead className="bg-gray-50 text-gray-500"><tr><th className="p-4">เวลา</th><th className="p-4">โต๊ะ</th><th className="p-4">ยอดเงิน</th><th className="p-4">สถานะ</th><th className="p-4 text-right">จัดการ</th></tr></thead>
                          <tbody>
                              {orders.map(o => (
                                  <tr key={o.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                                      ...
                                      <td className="p-4 text-right"><button onClick={() => deleteOrder(o.id)} className="text-red-400 hover:text-red-600 bg-red-50 hover:bg-red-100 p-2 rounded-lg transition-colors">🗑️</button></td>
                                  </tr>
                              ))}
                              {orders.length === 0 && <tr><td colSpan="5" className="p-8 text-center text-gray-400">ไม่พบข้อมูล</td></tr>}
                          </tbody>
                      </table>
                  </div>
              </div>
                  ...
                  {tables.map((t) => {
                      ...
                      return (
                          <div key={t.id} className={`relative p-6 rounded-2xl border-2 transition-all cursor-pointer group hover:shadow-lg ${isOccupied ? 'bg-white border-orange-500 shadow-md' : 'bg-gray-50 border-gray-200 border-dashed'}`}>
                              <div className="flex justify-between items-start mb-4"><span className={`text-4xl font-bold ${isOccupied ? 'text-gray-800' : 'text-gray-400'}`}>{t.table_number}</span><span className={`px-2 py-1 rounded-lg text-xs font-bold ${isOccupied ? 'bg-orange-100 text-orange-600' : 'bg-gray-200 text-gray-500'}`}>{isOccupied ? 'ไม่ว่าง' : 'ว่าง'}</span></div>
                              {isOccupied ? ( <div><p className="text-gray-500 text-sm mb-2">{tableActiveOrders.length} ออเดอร์</p><p className="text-2xl font-bold text-orange-600 mb-4">฿{total}</p><button onClick={() => openCheckBill(t)} className="w-full bg-orange-600 text-white py-2 rounded-xl font-bold shadow-lg shadow-orange-200 active:scale-95 transition-transform hover:bg-orange-700">💰 เช็คบิล</button></div> ) : ( <div className="h-[88px] flex items-center justify-center text-gray-300 text-sm">รอลูกค้า...</div> )}
                          </div>
                      );
                  })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                  {orders.filter(o => o.status !== 'completed').map((order) => (
                      <div key={order.id} className={`bg-white rounded-2xl shadow-sm border-l-4 overflow-hidden ${order.status === 'pending' ? 'border-orange-500' : order.status === 'cooking' ? 'border-blue-500' : 'border-green-500'}`}>
                          <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50"><h3 className="font-bold text-xl text-gray-800">โต๊ะ {order.table_number}</h3><span className="text-xs text-gray-400">#{order.id.toString().slice(-4)}</span></div>
                          <div className="p-4 space-y-2">{order.items.map((item, i) => ( <div key={i} className="flex justify-between items-center text-gray-600"><span>{item.name} {item.variant && <span className="text-orange-500 text-xs">({item.variant})</span>}</span><span className="font-bold bg-gray-100 px-2 rounded">x{item.quantity}</span></div> ))}</div>
                          <div className="p-4 pt-0">{order.status === 'pending' && <button onClick={()=>updateOrder(order.id, 'cooking')} className="w-full bg-orange-100 text-orange-700 py-2 rounded-xl font-bold hover:bg-orange-200">ทำอาหาร</button>}{order.status === 'cooking' && <button onClick={()=>updateOrder(order.id, 'served')} className="w-full bg-blue-100 text-blue-700 py-2 rounded-xl font-bold hover:bg-blue-200">เสิร์ฟ</button>}{order.status === 'served' && <button onClick={()=>updateOrder(order.id, 'completed')} className="w-full bg-green-100 text-green-700 py-2 rounded-xl font-bold hover:bg-green-200">จบงาน</button>}</div>
                      </div>
                  ))}
          ...
          {/* 🔥 5. SETTINGS VIEW (เพิ่มใหม่) */}
          {activeTab === 'settings' && (
              <div className="max-w-2xl mx-auto animate-fade-in">
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                      <div className="p-6 border-b border-gray-100"><h3 className="font-bold text-lg text-gray-800">🏪 ข้อมูลร้านค้า</h3><p className="text-sm text-gray-400">ข้อมูลนี้จะแสดงบนใบเสร็จ</p></div>
                      <div className="p-6 space-y-4">
                          <div><label className="block text-sm font-medium text-gray-700 mb-1">ชื่อร้าน</label><input className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-orange-500" value={settings.shop_name} onChange={e=>setSettings({...settings, shop_name: e.target.value})} /></div>
                          <div><label className="block text-sm font-medium text-gray-700 mb-1">ที่อยู่</label><textarea className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-orange-500 h-24" value={settings.shop_address} onChange={e=>setSettings({...settings, shop_address: e.target.value})} /></div>
                          <div><label className="block text-sm font-medium text-gray-700 mb-1">เบอร์โทรศัพท์</label><input className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-orange-500" value={settings.shop_phone} onChange={e=>setSettings({...settings, shop_phone: e.target.value})} /></div>
                      </div>
                  </div>

                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                      <div className="p-6 border-b border-gray-100"><h3 className="font-bold text-lg text-gray-800">🧾 การตั้งค่าภาษี & ใบเสร็จ</h3></div>
                      <div className="p-6 space-y-4">
                          <div><label className="block text-sm font-medium text-gray-700 mb-1">อัตราภาษี (VAT %)</label><input type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-orange-500" value={settings.vat_rate} onChange={e=>setSettings({...settings, vat_rate: e.target.value})} /></div>
                          <div><label className="block text-sm font-medium text-gray-700 mb-1">ข้อความท้ายใบเสร็จ</label><input className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-orange-500" value={settings.receipt_footer} onChange={e=>setSettings({...settings, receipt_footer: e.target.value})} /></div>
                      </div>
                  </div>
      ...
      {showBillModal && currentBillTable && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
                  <div className="bg-orange-600 p-6 text-white text-center"><h3 className="text-2xl font-bold">โต๊ะ {currentBillTable.table_number}</h3><p className="opacity-80 text-sm">สรุปยอดชำระเงิน</p></div>
                  <div className="p-6 max-h-[300px] overflow-y-auto bg-gray-50">{billItems.map((item, idx) => ( <div key={idx} className="flex justify-between py-2 border-b border-gray-100 last:border-0 text-sm text-gray-600"><span>{item.name} x{item.quantity}</span><span className="font-bold text-gray-800">{item.price}</span></div> ))}</div>
                  <div className="p-6 pt-0 bg-white border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                      <div className="flex justify-between items-center py-4"><span className="text-gray-500">ยอดรวมทั้งสิ้น</span><span className="text-3xl font-black text-orange-600">฿{billTotal}</span></div>
                      <div className="mb-4"><input type="number" value={cashReceived} onChange={(e) => setCashReceived(e.target.value)} className="w-full bg-gray-100 text-center text-2xl font-bold p-3 rounded-xl outline-none focus:ring-2 focus:ring-green-500 text-gray-800" placeholder="รับเงินมา..." autoFocus /></div>
                      {cashReceived && ( <div className="flex justify-between items-center mb-4 p-3 bg-green-50 rounded-xl text-green-700"><span className="font-bold">เงินทอน</span><span className="text-xl font-black">฿{parseFloat(cashReceived) - billTotal}</span></div> )}
                      <div className="flex gap-2"><button onClick={() => setShowBillModal(false)} className="flex-1 py-3 rounded-xl text-gray-500 hover:bg-gray-100">ยกเลิก</button><button onClick={confirmPayment} disabled={!cashReceived || parseFloat(cashReceived) < billTotal} className={`flex-1 py-3 rounded-xl font-bold text-white shadow-lg ${!cashReceived || parseFloat(cashReceived) < billTotal ? 'bg-gray-300' : 'bg-green-600 hover:bg-green-500 shadow-green-200'}`}>ยืนยัน</button></div>
                  </div>
              </div>
          </div>
      )}
