...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState('menu'); // ตั้งค่าเริ่มต้นเป็นหน้าเมนู จะได้ไม่ต้องกดสลับบ่อย
  const [tables, setTables] = useState([]);
  const [orders, setOrders] = useState([]);
  const [menus, setMenus] = useState([]);
  
  // State สำหรับเพิ่มเมนู (รองรับราคาพิเศษ)
  ...
  const fetchData = async () => {
    ...
    if (o) setOrders(o);
    const { data: m } = await supabase.from('restaurant_menus').select('*').order('id'); // เรียงตาม ID จะได้ไม่งง
    if (m) setMenus(m);
  };
  ...
  // Menu Actions (อัปเกรดใหม่)
  const handleAddMenu = async (e) => { 
      e.preventDefault(); 
      // แปลงค่าว่างให้เป็น null สำหรับราคาพิเศษ
      const payload = {
          ...newMenu,
          price_special: newMenu.price_special ? newMenu.price_special : null
      };
      const { error } = await supabase.from('restaurant_menus').insert([payload]); 
      if (error) alert("Error: " + error.message);
      else {
          setNewMenu({ name: '', price: '', price_special: '', category: 'Noodles', image_url: '' }); 
          fetchData(); 
      }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] p-6 text-white font-sans">
      ...
      {/* --- ส่วนจัดการเมนู (แก้ไขใหม่ให้ตรงกับฐานข้อมูล) --- */}
      {activeTab === 'menu' && (
        <div className="animate-fade-in max-w-5xl mx-auto">
           ...
           <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 shadow-lg mb-8">
             ...
             <form onSubmit={handleAddMenu} className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                <div className="md:col-span-2">
                    <label className="text-xs text-gray-400 mb-1 block">ชื่อเมนู</label>
                    <input className="w-full bg-gray-900 border border-gray-600 text-white px-3 py-2 rounded focus:border-blue-500 outline-none" 
                        placeholder="เช่น เส้นเล็กน้ำตก" 
                        value={newMenu.name} onChange={e=>setNewMenu({...newMenu, name: e.target.value})} required />
                </div>
                <div className="md:col-span-1">
                    <label className="text-xs text-gray-400 mb-1 block">ราคาปกติ</label>
                    <input type="number" className="w-full bg-gray-900 border border-gray-600 text-white px-3 py-2 rounded focus:border-blue-500 outline-none" 
                        placeholder="0" 
                        value={newMenu.price} onChange={e=>setNewMenu({...newMenu, price: e.target.value})} required />
                </div>
                <div className="md:col-span-1">
                    <label className="text-xs text-yellow-400 mb-1 block">ราคาพิเศษ (ถ้ามี)</label>
                    <input type="number" className="w-full bg-gray-900 border border-yellow-600/50 text-white px-3 py-2 rounded focus:border-yellow-500 outline-none" 
                        placeholder="0" 
                        value={newMenu.price_special} onChange={e=>setNewMenu({...newMenu, price_special: e.target.value})} />
                </div>
                <div className="md:col-span-1">
                    <label className="text-xs text-gray-400 mb-1 block">หมวดหมู่</label>
                    <select className="w-full bg-gray-900 border border-gray-600 text-white px-3 py-2 rounded focus:border-blue-500 outline-none" 
                        value={newMenu.category} onChange={e=>setNewMenu({...newMenu, category: e.target.value})}>
                        <option value="Noodles">🍜 ก๋วยเตี๋ยว</option>
                        <option value="GaoLao">🍲 เกาเหลา</option>
                        <option value="Sides">🍚 ของทานเล่น/ข้าว</option>
                        <option value="Drinks">🥤 เครื่องดื่ม</option>
                    </select>
                </div>
                <div className="md:col-span-1">
                    <button type="submit" className="w-full bg-green-600 hover:bg-green-500 text-white py-2 rounded-lg font-bold shadow-lg transition-transform active:scale-95 h-[42px]">
                        + เพิ่ม
                    </button>
                </div>
                {/* ช่องกรอก URL รูปภาพ (เผื่ออยากใส่) */}
                <div className="md:col-span-6 mt-2">
                     <input className="w-full bg-gray-900 border border-gray-700 text-gray-400 text-sm px-3 py-2 rounded" 
                        placeholder="ลิ้งค์รูปภาพ (URL) - ถ้าไม่ใส่จะแสดงไอคอนแทน"
           ...
           <div className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700">
             <table className="w-full text-left border-collapse">
               <thead>
                 <tr className="bg-gray-900 text-gray-400 text-xs uppercase border-b border-gray-700">
                   ...
                   <tr key={m.id} className="hover:bg-gray-700/50 border-b border-gray-700 last:border-0 transition-colors">
                      <td className="px-4 py-3 w-16">
                          <div className="w-10 h-10 bg-gray-700 rounded overflow-hidden flex items-center justify-center">
                              {m.image_url ? <img src={m.image_url} className="w-full h-full object-cover"/> : "🍽️"}
                          </div>
                      </td>
                      <td className="px-4 py-3 font-medium">{m.name}</td>
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-1 rounded border ${
                            m.category === 'Noodles' ? 'bg-orange-900/30 border-orange-500/50 text-orange-300' : 
                            m.category === 'GaoLao' ? 'bg-blue-900/30 border-blue-500/50 text-blue-300' : 
                            'bg-gray-700 border-gray-600 text-gray-300'
                        }`}>
                            {m.category === 'Noodles' ? 'ก๋วยเตี๋ยว' : m.category === 'GaoLao' ? 'เกาเหลา' : m.category === 'Sides' ? 'ของทานเล่น' : m.category}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                          <div className="flex flex-col text-sm">
                              <span className="text-gray-300">ธรรมดา: {m.price}</span>
                              {m.price_special && <span className="text-yellow-400 font-bold">พิเศษ: {m.price_special}</span>}
                          </div>
                      </td>
                      <td className="px-4 py-3 text-right">
                          <button onClick={() => deleteMenu(m.id)} className="text-red-400 hover:text-red-300 border border-red-500/30 px-3 py-1 rounded text-xs hover:bg-red-500/10">ลบ</button>
                      </td>
                   </tr>
                 ))}
               </tbody>
